...
def process_pair(src_file, ref_file, output_file, tokenizer, src_lang, tgt_lang):
    ...
    assert len(src_lines) == len(
        ref_lines
    ), f"Line count mismatch: {src_file} and {ref_file}"
...
def main():
    ...
    process_pair(
        os.path.join(input_dir, "test.de"),
        os.path.join(input_dir, "test.en"),
        os.path.join(output_dir, "de-en.jsonl"),
        tokenizer,
        "de",
        "en",
    )
    ...
    process_pair(
        os.path.join(input_dir, "test.en"),
        os.path.join(input_dir, "test.de"),
        os.path.join(output_dir, "en-de.jsonl"),
        tokenizer,
        "en",
        "de",
    )
